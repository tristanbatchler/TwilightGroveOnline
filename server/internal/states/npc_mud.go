...
func (n *NpcMud) Name() string {
	return "NpcMud"
}

func (n *NpcMud) SetClient(client central.ClientInterfacer) {
	n.client = client
	loggingPrefix := fmt.Sprintf("Client %d [%s]: ", client.Id(), n.Name())
	n.logger = log.New(log.Writer(), loggingPrefix, log.LstdFlags)
}

func (n *NpcMud) OnEnter() {
	n.levelId = 1
	n.actor = objs.NewActor(n.levelId, 25, -1, "Mud", 0)
	n.actor.IsNpc = true
	...
	n.shop.AddItem(*items.Logs, 100)
	n.shop.AddItem(*items.BronzeHatchet, 10)
}

func (n *NpcMud) HandleMessage(senderId uint32, message packets.Msg) {
...
func (n *NpcMud) handleActorInfo(senderId uint32, _ *packets.Packet_Actor) {
...
func (n *NpcMud) handleInteractWithNpcRequest(senderId uint32, message *packets.Packet_InteractWithNpcRequest) {
	if senderId == n.client.Id() {
...
func (n *NpcMud) handleBuyRequest(senderId uint32, message *packets.Packet_BuyRequest) {
	if senderId == n.client.Id() {
...
func (n *NpcMud) handleSellRequest(senderId uint32, message *packets.Packet_SellRequest) {
	if senderId == n.client.Id() {
...
func (n *NpcMud) removeFromOtherInLevel(clientId uint32) {
...
func (n *NpcMud) isOtherKnown(otherId uint32) bool {
...
func (n *NpcMud) moveLoop(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			return
...
func (n *NpcMud) move(dx, dy int32) {
